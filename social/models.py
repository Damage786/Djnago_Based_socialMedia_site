...
class Profile(models.Model):
    ...
    def get_suggested_users(self):
        # Example logic: Suggest random users excluding the current user and those already followed
        return Profile.objects.exclude(user=self.user).exclude(followed_by=self).order_by('?')[:5]
...
def create_profile(sender, instance, created, **kwargs):
    if created:
        ...
        user_profile.profile_image = 'https://t4.ftcdn.net/jpg/00/64/67/27/360_F_64672736_U5kpdGs9keUll8CRQ3p3YaEv2M6qkVY5.jpg'  # Set the default profile image path
        user_profile.save()
...
class Message(models.Model):
    ...
    def get_content_with_emojis(self):
